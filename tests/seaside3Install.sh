...
eval \`[ (self hasErrors or: [ self unexpectedFailures size > 0 or: [ (self unexpectedPasses reject: [ :each | each class = WAWebDriverFunctionalTestCase and: [ each selector = #'testCORSFilterFunctionalTest' ] ]) size > 0 ] ]) ifTrue: [ self error: 'Tests failed' ] ] on: Warning do: [ :ex | ex resume: true ]\`
EOF
status=$?
